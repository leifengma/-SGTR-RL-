...
model = keras.models.load_model(
    r'M:\work\project_program\bishe\pyproject\RL_pid_control_critic_actor\强化学习+Lstm\一回路温度控制\温度模型\模型\best_model_epoch124.hdf5')
#
...
def eval_policy(policy, env_name, seed, eval_episodes=1):
    ...
    for _ in range(eval_episodes):
        ...
        for i in range(600):
            ...
            state_, reward, done, _ = eval_env.step(action)
            state=state_
            avg_reward += reward
...
def train():
    ...
    parser.add_argument("--env", default="SGTR")  # OpenAI gym environment name
    ...
    parser.add_argument("--discount", default=0.90)  # Discount factor
    parser.add_argument("--tau", default=0.005)  # Target network update rate
    parser.add_argument("--policy_noise", default=0.002)  # Noise added to target policy during critic update
    parser.add_argument("--noise_clip", default=0.005)  # Range to clip target policy noise
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t < args.start_timesteps:
            ...
            ).clip(action_min, max_action)

        # Perform action
        next_state, reward, done, _ = env.step(action)
        done=True if   episode_timesteps == 600 else False
        ...
        if done:
            ...
            # Reset environment
            state, done = env.reset(), False
            reward_save.append(episode_reward)
            episode_reward = 0
            episode_timesteps = 0
            episode_num += 1
        ...
        if (t + 1) % args.eval_freq == 0:
            evl_reward = eval_policy(policy, args.env, args.seed)

            if evl_reward > max_reward:
                policy.save(f"./models/{file_name+str(time.localtime(time.time()))}")
                print("saving model")
                max_reward=evl_reward

    np.save(r'总回报记录.npy',reward_save)
